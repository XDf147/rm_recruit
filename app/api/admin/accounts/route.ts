...
export function GET(request: NextRequest) {
  ...
  return NextResponse.json({ accounts: rows.map((row) => ({ id: row.id, username: row.username, displayName: row.display_name, role: row.role, groupName: row.group_name, active: Boolean(row.active), createdAt: row.created_at })) });
}

export async function POST(request: NextRequest) {
  const admin = getRequestAdmin(request);
  if (!admin || admin.role !== "captain") return NextResponse.json({ error: "仅队长可以创建账号" }, { status: 403 });
  const payload = (await request.json().catch(() => ({}))) as { username?: string; password?: string; displayName?: string; groupName?: string };
  const username = payload.username?.trim() ?? "";
  const displayName = payload.displayName?.trim() ?? "";
  const password = payload.password ?? "";
  const groupName = payload.groupName?.trim() ?? "";
  if (!/^[a-zA-Z0-9_.-]{3,32}$/.test(username)) return NextResponse.json({ error: "账号需为 3–32 位字母、数字、点、下划线或连字符" }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({ account: { id, username, displayName, role: "leader", groupName, active: true, createdAt: now } }, { status: 201 });
  } catch (error) {
    if (error instanceof Error && error.message.includes("UNIQUE")) return NextResponse.json({ error: "该账号已存在" }, { status: 409 });
